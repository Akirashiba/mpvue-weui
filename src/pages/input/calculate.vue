<template>
  <div class="calculate">
    ...
    <div class="weui-cells weui-cells_after-title">
      <div 
        v-for="output_prop in outPutList"
        :key="output_prop"
        :class="{'weui-cell': true, 'weui-cell_warn': outputs_error[output_prop]}"
      >
        <div class="weui-cell__hd">
        ...
        <div v-if="outputs_error[output_prop]" class="weui-cell__ft">
          <icon type="warn" size="23" color="#E64340"></icon>
        </div>
      </div>
    </div>
    <div class="weui-btn-area">
      <button class="weui-btn mini-btn" size="mini" type="primary" @click="reset">重置</button>
      <button class="weui-btn mini-btn ml2em" size="mini" type="primary" @click="calculation">确定</button>
    </div>
...
export default {
  ...
  methods: {
    noCheckTowelCalculate() {
      let k = parseFloat(this.inputs_val["k_val"]);
      let x1 = parseInt(this.inputs_val["x1"]);
      let y1 = parseInt(this.inputs_val["y1"]);
      let x2 = parseInt(this.inputs_val["x2"]);
      let y2 = parseInt(this.inputs_val["y2"]);
      let x3 = parseInt(this.inputs_val["x3"]);
      let y3 = parseInt(this.inputs_val["y3"]);
      let cx = parseInt(this.inputs_val["cx"]);
      let cy = parseInt(this.inputs_val["cy"]);
      let ax = parseInt(this.inputs_val["ax"]);
      let ay = parseInt(this.inputs_val["ay"]);
      let l = cx - ax;
      k /= 100000;
      this.outputs_val["fm"] = k * l * l;
      let tanb = (cy - ay) / l;
      if (x1 > 0) {
        this.outputs_val["h1"] = tanb * (x1 - ax) - 4 * k * (x1 - ax) * (cx - x1) + ay - y1;
      }
      if (x2 > 0) {
        this.outputs_val["h2"] = tanb * (x2 - ax) - 4 * k * (x2 - ax) * (cx - x2) + ay - y2;
      }
      if (x3 > 0) {
        this.outputs_val["h3"] = tanb * (x3 - ax) - 4 * k * (x3 - ax) * (cx - x3) + ay - y3;
      }
    ...
    CheckTowelCalculate() {
      ...
      let cx = parseInt(this.inputs_val["cx"]);
      let cy = parseInt(this.inputs_val["cy"]);
      let bx = parseInt(this.inputs_val["bx"]);
      let by = parseInt(this.inputs_val["by"]);
      let ax = parseInt(this.inputs_val["ax"]);
      let ay = parseInt(this.inputs_val["ay"]);
      let l1 = bx - ax;
      let l2 = cx - bx;
      ...
      let tan2 = (cy - by) / l2;
      if (x1 > bx) {
        this.outputs_val["h1"] = tan2 * (x1 - bx) - 4 * k * (x1 - bx) * (cx - x1) + by - y1;
      } else if (x1 > 0) {
        this.outputs_val["h1"] = tan1 * (x1 - ax) - 4 * k * (x1 - ax) * (bx - x1) + ay - y1;
      }
      if (x2 > bx) {
        this.outputs_val["h2"] = tan2 * (x2 - bx) - 4 * k * (x2 - bx) * (cx - x2) + by - y2;
      } else if (x2 > 0) {
        this.outputs_val["h2"] = tan1 * (x2 - ax) - 4 * k * (x2 - ax) * (bx - x2) + ay - y2;
      }
      if (x3 > bx) {
        this.outputs_val["h3"] = tan2 * (x3 - bx) - 4 * k * (x3 - bx) * (cx - x3) + by - y3;
      } else if (x3 > 0) {
        this.outputs_val["h3"] = tan1 * (x3 - ax) - 4 * k * (x3 - ax) * (bx - x3) + ay - y3;
      }
    ...
    reset() {
      for (var inputProp in this.inputs_name) {
        if (rules[inputProp]) {
          this.inputs_val[inputProp] = rules[inputProp]["default"];
          this.inputs_error[inputProp] = false;
        }
      }
      for (let outputProp in this.outputs_name) {
        if (this.outputs_val[outputProp] !== undefined) {
          this.outputs_val[outputProp] = 0.0;
          this.outputs_error[outputProp] = false;
        }
      }
    },
    validateOutput() {
      if (this.outputs_val["ad"] > 0.46) {
        this.outputs_error["ad"] = true;
      }
      if (this.outputs_val["cd"] > 0.46) {
        this.outputs_error["cd"] = true;
      }
    }
  }
}
